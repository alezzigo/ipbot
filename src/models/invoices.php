...
/**
 * Deduct amount from merged and additional invoices
 *
 * @param array $invoiceData
 * @param integer $amount
 * @param array $invoiceDeductions
 ...
 * @return array $response
 */
	protected function _deductFromInvoices($invoiceData, $amount, $invoiceDeductions = array()) {
		if (!empty($invoiceData)) {
			$remainder = min(0, round(($invoiceData['amount_paid'] + $amount) * 100) / 100);
			$amount = max(($invoiceData['amount_paid'] * -1), round(($amount - $remainder) * 100) / 100);
			$invoiceDeduction = array(
			...
			if ($remainder < 0) {
				...
				if (!empty($additionalInvoice['count'])) {
					$invoiceDeductions = $this->_deductFromInvoices($additionalInvoice['data'][0], $remainder, $invoiceDeductions);
				}
			}
		}
