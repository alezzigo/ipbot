	...
	sendRequest(function(response) {
		...
		if (
			...
			response.data.map(function(item, index) {
				...
				ordersData += '<label class="label ' + item.status + '">' + capitalizeString(item.status) + '</label>' + (pendingOrderChange ? '<label class="label">Pending Order ' + (item.quantity_pending >= item.quantity ? 'Upgrade' : 'Downgrade') + '</label>' : '');
				ordersData += '</div>';
				ordersData += '</div>';
...
var processUpgrade = function(windowName, windowSelector, upgradeValue = 1) {
	...
	sendRequest(function(response) {
		...
		if (response.message.status === 'success') {
			...
			upgradeData += '<p class="no-margin-bottom">' + response.data.merged.invoice.payment_currency_symbol + response.data.merged.order.price_pending.toLocaleString(false, {minimumFractionDigits: 2}) + ' ' + response.data.merged.invoice.payment_currency_name + ' for ' + response.data.merged.order.interval_value_pending + ' ' + response.data.merged.order.interval_type_pending + (response.data.merged.order.interval_value_pending !== 1 ? 's' : '') + '</p>';
			upgradeData += '<div class="item-link-container"></div>';
			upgradeData += '</div>';
			upgradeData += '<h2>' + (upgradeValue > 0 ? 'Upgraded': 'Merged') + ' Invoice Pricing Details</h2>';
			upgradeData += '<p><strong>Subtotal</strong><br>' + response.data.merged.invoice.payment_currency_symbol + response.data.merged.invoice.subtotal_pending.toLocaleString(false, {minimumFractionDigits: 2}) + ' ' + response.data.merged.invoice.payment_currency_name + '</p>';
			upgradeData += '<p><strong>Shipping</strong><br>' + response.data.merged.invoice.payment_currency_symbol + response.data.merged.invoice.shipping_pending.toLocaleString(false, {minimumFractionDigits: 2}) + ' ' + response.data.merged.invoice.payment_currency_name + '</p>';
			upgradeData += '<p><strong>Tax</strong><br>' + response.data.merged.invoice.payment_currency_symbol + response.data.merged.invoice.tax_pending.toLocaleString(false, {minimumFractionDigits: 2}) + ' ' + response.data.merged.invoice.payment_currency_name + '</p>';
			upgradeData += '<p><strong>Total</strong><br>' + response.data.merged.invoice.payment_currency_symbol + response.data.merged.invoice.total_pending.toLocaleString(false, {minimumFractionDigits: 2}) + ' ' + response.data.merged.invoice.payment_currency_name + '</p>';
			upgradeData += '<p><strong>Amount Paid</strong><br><span class="paid">' + response.data.merged.invoice.payment_currency_symbol + response.data.merged.invoice.amount_paid.toLocaleString(false, {minimumFractionDigits: 2}) + ' ' + response.data.merged.invoice.payment_currency_name + '</span></p>';
			upgradeData += '<p><strong>Remaining Amount Due</strong><br>' + response.data.merged.invoice.payment_currency_symbol + response.data.merged.invoice.remainder_pending.toLocaleString(false, {minimumFractionDigits: 2}) + ' ' + response.data.merged.invoice.payment_currency_name + '</p>';
			upgradeData += '</div>';
