...
var processSelection = (checkbox) => {
	checkbox.target.hasAttribute('checked') ? checkbox.target.removeAttribute('checked') : checkbox.target.setAttribute('checked', '');

	var totalChecked = selectAllElements('.checkbox[checked]').length,
		totalResults = document.querySelector('.checked-container .total-results').innerHTML;
	document.querySelector('.checked-container .total-checked').innerHTML = totalChecked;
